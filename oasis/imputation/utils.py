...
def extract_label_tokens(text: str | None) -> List[str]:
    ...
    - <LBL:INCEL_SLANG> (with angle brackets)
    - LBL:INCEL_SLANG (bare format)
    """
    if not text:
        return []
    tokens = []
    for match in LABEL_TOKEN_PATTERN.finditer(text):
        # Group 1 is for <LBL:...>, group 2 is for bare LBL:...
        label = match.group(1) or match.group(2)
        if label:
            tokens.append(f"LBL:{label}")
    return tokens
